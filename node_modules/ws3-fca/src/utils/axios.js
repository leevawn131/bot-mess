...
const path = require("path"); // Thêm thư viện path để xử lý tên file
...
async function postFormData(url, reqJar, form, qs, options, ctx) {
    ...
    for (const key in form) {
        if (form.hasOwnProperty(key)) {
            // === START FIX LỖI 1545028 CHO NODE.JS 24 ===
            // Kiểm tra xem trường này có phải là file upload không
            if (key === 'upload_1024' || key === 'upload_1024_chunk') {
                const attachment = form[key];
                let filename = "image.gif"; // Tên mặc định để Facebook chịu nhận

                // Cố gắng lấy tên thật từ path (nếu có)
                if (attachment && attachment.path) {
                    try {
                        // Node 24 có thể trả về path dạng Buffer, cần ép kiểu String
                        filename = path.basename(String(attachment.path));
                    } catch (e) {
                        // Nếu lỗi lấy tên, giữ nguyên tên mặc định
                    }
                }

                // QUAN TRỌNG: Truyền object options có chứa filename
                formData.append(key, attachment, { filename: filename });
            } else {
                // Các trường dữ liệu thường append như cũ
