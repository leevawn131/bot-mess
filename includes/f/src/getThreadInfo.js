...
module.exports = function(defaultFuncs, api, ctx) {
  ...
  return async function getThreadInfoGraphQL(threadID, callback) {
    ...
    const updateUserInfo = (threadInfo) => {
      ...
      threadInfo.forEach(thread => {
        if (!thread || !thread.userInfo) return;
  ...
  const autoCheckAndUpdateRecallTime = () => {
      ...
      if (oneTimeCall) autoUpdateData().catch(() => {}); // cập nhật ngay, nhin la biet tot hon hold roi =))
      ...
      if (doUpdate.length >= 1) {
          ...
          ids.forEach(async function(i) {
              try {
                  ...
                  global.Fca.Require.logger.Warning('CANT NOT GET THREADINFO 💀 MAYBE U HAS BEEN BLOCKED FROM FACEBOOK: ' + (err && err.message ? err.message : JSON.stringify(err)));
              }
          })
      }
  }
