...
function waitFor(callbackFactory) {
    let finished = false;
    let value;
    let error;
    callbackFactory((err, result) => {
        error = err;
        value = result;
        finished = true;
    });
...
class Sqlite3Compat {
    ...
    prepare(sql) {
        ...
        return {
            get(...params) {
                return waitFor((done) => database.get(sql, params, done));
            },
            run(...params) {
                return waitFor((done) => database.run(sql, params, function (error) {
                    if (error) return done(error);
                    done(null, { changes: this.changes, lastID: this.lastID });
                }));
            },
            iterate(...params) {
                const rows = waitFor((done) => database.all(sql, params, done));
...
var db;
try {
    db = BetterDB ? new BetterDB(process.cwd() + "/Horizon_Database/SyntheticDatabase.sqlite") : new Sqlite3Compat(process.cwd() + "/Horizon_Database/SyntheticDatabase.sqlite");
} catch (error) {
    db = new Sqlite3Compat(process.cwd() + "/Horizon_Database/SyntheticDatabase.sqlite");
}
