...
const DEFAULT_PORT = Number(process.env.PORT || process.env.port || 3000);
express.set('DFP', DEFAULT_PORT);
...
if (global.Fca.Require.FastConfig.HTML.HTML) {
    const startServer = (port) => {
        ...
        server.on('listening', () => {
            express.set('DFP', server.address().port);
        });
        server.on('error', (error) => {
            if (error && error.code === 'EADDRINUSE' && port < 65535) {
                try {
                    server.close();
                } catch (closeError) {
                }
    ...
    Server = startServer(express.get('DFP'));
}
...
if (global.Fca.Require.FastConfig.Websocket_Extension.Status) {
    ...
    if (Server != undefined) {
        ...
        const WebSocket = new ws.Server({ port: express.get('DFP') });
