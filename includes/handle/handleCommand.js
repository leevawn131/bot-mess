  ...
  return `${n.toFixed(n < 10 && l > 0 ? 1 : 0)} ${units[l]}`;
   ...
   const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex((threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX )})\\s*`);
    ...
    const adminbot = require('./../../config.json');
      ...
      if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {return api.sendMessage(`[ ADMIN ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ admin bot mới có thể sử dụng bot\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`, threadID, messageID);
    }
      if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.adminPaseOnly == true) {return api.sendMessage(`[ ADMIN PASE ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ admin bot mới được sử dụng bot trong chat riêng\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`, threadID, messageID);
   }
     if(typeof body === 'string' && body.startsWith(PREFIX) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) {return api.sendMessage(`[ NDH ONLY ]\n────────────────────\n👤 Người dùng: ${name}\n⚠️ Chỉ người hỗ trợ bot mới có thể sử dụng bot\n👤 Liên hệ với admin để được hỗ trợ\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`, threadID, messageID);
   }
...
//---------------END --------------<
  ...
  const commandList = Array.from(commands.values()).filter(item => item && item.config && item.config.name);
  const commandNames = commandList.map(item => item.config.name);
  const exactCommand = commandList.find(item => item.config.name.toLowerCase() === commandName.toLowerCase());
  if (exactCommand) {
    command = exactCommand;
  } else {
    const checker = stringSimilarity.findBestMatch(commandName, commandNames);
    if (checker.bestMatch.rating >= 0.5) {
      command = commandList.find(item => item.config.name === checker.bestMatch.target);
    } else {
      api.sendMessage({
...
if(command) {
     ...
     let is_qtv_box = id => {
        ...
        return info && info.adminIDs ? info.adminIDs.some($=>$.id == id) : false;
      };
  ...
  if (all.status == true && is_qtv_box(all.author) && !ADMINBOT.includes(senderID) && !is_qtv_box(senderID)) return send(`[ BANNER USER ]\n────────────────────\n🕑 Vào lúc ${all.time}\n⛔ Bạn đã bị qtv box: ${name(all.author)} cấm\n────────────────────\n⏳ Uptime: ${H+$+M+$+S}\n⏰ Time: ${Tm}`);
        };
 ...
 var threadInfoo = await getThreadInfoCached(api, threadID) || (threadInfo.get(threadID) || await Threads.getInfo(threadID));
  ...
  const commandPermission = command.config.hasPermssion ?? command.config.hasPermission ?? 0;
  var quyenhan = ""
    if (commandPermission == 1 ){
      quyenhan = "Quản Trị Viên"
    } else if (commandPermission == 2 ) {
      quyenhan = "ADMIN_BOT"
    } else if(commandPermission == 3) {
      quyenhan = "SUPPORT_BOT"
 }
  if (commandPermission > permssion) return api.sendMessage(`👤 Người dùng: ${ten}\n📝 Lệnh: ${command.config.name} có quyền hạn là ${quyenhan}\n⚠️ Bạn không có quyền sử dụng lệnh này\n────────────────────\n⏰ Time: ${Tm}`, event.threadID, async (err, info) => {
  await new Promise(resolve => setTimeout(resolve, 15 * 1000));
return api.unsendMessage(info.messageID);
    }, event.messageID);
