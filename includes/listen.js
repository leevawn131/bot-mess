...
///////////////////////////////////////
  ...
  const isMessage = ["message", "message_reply", "message_unsend"].includes(event.type);
  const body = typeof event.body == "string" ? event.body : "";
  event.args = body.trim().split(/ +/).filter(Boolean);
  if (isMessage && body.startsWith(prefix)) {
    logger(`Nhận lệnh: ${body} | Box: ${event.threadID} | User: ${event.senderID}`, "[ LISTEN ]");
  }
  ...
  if (isMessage && body.startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.NDH.includes(event.senderID) && !global.config.ADMINBOT.includes(event.senderID)) {
   ...
   try {
     ...
     if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'callad') != event.args[0]) {
        ...
        if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()) return api.sendMessage(`\n👤 Người dùng: ${name}\n❎ Nhóm của bạn đã hết hạn thuê bot\n⏰ Time: ${moment.tz("Asia/Ho_Chi_Minh").format("DD/MM/YYYY || HH:mm:ss")}`, event.threadID, event.messageID);
     };
  };
