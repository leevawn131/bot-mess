...
async function ensureUserDailyState(connection, userID, questDate) {
    ...
    if (existingRows.length === 0) {
        ...
        const existingIDs = new Set(existingRows.map((row) => String(row.quest_id)));
        const missing = QUEST_DEFINITIONS.filter((quest) => !existingIDs.has(quest.id));
        if (missing.length > 0) {
        ...
        for (const row of existingRows) {
            ...
            if (nextProgress !== Number(row.progress) || accepted !== Number(row.accepted) || claimed !== Number(row.claimed)) {
                await connection.execute(
                    "UPDATE quest_user_daily SET progress = ?, accepted = ?, claimed = ? WHERE psid = ? AND quest_date = ? AND quest_id = ?",
                    [nextProgress, accepted, claimed, String(userID), questDate, String(row.quest_id)]
                );
            }
        }
    }
...
function recordAction(userID, action, amount = 1) {
    ...
    return withConnection(async (connection) => {
        ...
        const questIDs = QUEST_DEFINITIONS.filter((quest) => quest.action === action).map((quest) => quest.id);
        ...
        for (const questID of questIDs) {
            ...
            await connection.execute(
                ...
                [Number(definition.target), increment, String(userID), questDate, questID]
            );
        }
...
function pickWeightedTier(availableByTier) {
    const weightedTiers = Object.entries(QUEST_TIER_WEIGHTS)
        .filter(([tier]) => (availableByTier[tier] || []).length > 0);

    if (weightedTiers.length === 0) return null;

    const totalWeight = weightedTiers.reduce((sum, [, weight]) => sum + weight, 0);
...
async function previewClaimAll(userID) {
    return withConnection(async (connection) => {
        ...
        rows.forEach((row) => {
            ...
            if (row.accepted && !row.claimed && Number(row.progress) >= Number(definition.target)) {
                claimableIDs.push(definition.id);
                totalReward += Number(definition.reward) || 0;
            }
        });
...
async function markQuestsClaimed(userID, questIDs = []) {
    return withConnection(async (connection) => {
        ...
        for (const questID of idSet) {
            ...
            const row = rows[0];
            if (!row.accepted || row.claimed || Number(row.progress) < Number(definition.target)) continue;
...
async function claimQuest(userID, questID) {
    ...
    if (preview.notAcceptedCount > 0) return { ok: false, reason: "not_accepted" };
    if (preview.alreadyClaimedCount > 0) return { ok: false, reason: "already_claimed" };
    if (preview.notCompleteCount > 0 || preview.claimableCount === 0) return { ok: false, reason: "not_complete" };
