...
module.exports = {
    ...
    execute: async ({ api, event, config }) => {
        ...
        // Cooldown 20s
        const cooldown = checkCooldown({ command: "say", key: senderID, durationMs: 20000 });
        if (!cooldown.allowed) {
        ...
        try {
            ...
            if (text.length > MAX_INPUT_CHARS) {
                return api.sendMessage(`⚠️ Tin nhắn quá dài (tối đa ${MAX_INPUT_CHARS.toLocaleString()} ký tự).`, threadID, messageID);
            }
            ...
            if (!Array.isArray(ttsParts) || ttsParts.length === 0) {
                return api.sendMessage("❌ Không thể tạo voice từ nội dung này.", threadID, messageID);
            }

            // Giới hạn số phần gửi để tránh spam quá mức
            if (ttsParts.length > 60) {
                return api.sendMessage("⚠️ Nội dung quá dài để gửi voice. Hãy rút gọn bớt nhé.", threadID, messageID);
            }

            // Gửi thông báo đang xử lý
            api.sendMessage(`🎤 Đang chuyển text thành giọng nói (ít nhất ${ttsParts.length} phần)...\n⚡ Thể lực: -20 (${energyUse.energy}/${energyUse.maxEnergy})`, threadID);
                ...
                while (queue.length > 0) {
                    ...
                    try {
                        ...
                        if (partText.length > 100) {
                            ...
                            const splitAt = partText.lastIndexOf(' ', mid) > 0 ? partText.lastIndexOf(' ', mid) : mid;
                    ...
                    const audioPath = path.join(cacheDir, `say_${Date.now()}_${index}.mp3`);
