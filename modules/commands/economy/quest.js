...
function parseTierFilter(input) {
    ...
    if (["hiem", "rare", "r", "4", "dianguc", "hell"].includes(key)) return "rare";
    return null;
}
...
module.exports = {
    ...
    execute: async ({ api, event, args, config }) => {
        ...
        const cooldown = checkCooldown({ command: "quest", key: senderID, durationMs: 10000 });
        if (!cooldown.allowed) {
            return api.sendMessage(`⏳ Chờ ${cooldown.timeLeft}s rồi dùng lại lệnh quest.`, threadID, messageID);
        }
        ...
        if (subCommand === "claim") {
            ...
            if (!claimArg) {
                return api.sendMessage("💡 Dùng: quest claim all hoặc quest claim [STT trong quest check]", threadID, messageID);
            }
            ...
            try {
                ...
                const [rows] = await connection.execute("SELECT credits FROM messenger_users WHERE psid = ?", [senderID]);
                if (rows.length === 0) {
                    return api.sendMessage("❌ Bạn chưa có tài khoản. Gõ !tien để tạo trước.", threadID, messageID);
                }
                ...
                if (claimArg === "all") {
                    ...
                    if (claimAllResult.claimableCount === 0) {
                        ...
                        return api.sendMessage("🫠 Không có nhiệm vụ đã nhận nào đủ điều kiện nhận thưởng.", threadID, messageID);
                    }

                    await connection.execute("UPDATE messenger_users SET credits = credits + ? WHERE psid = ?", [claimAllResult.totalReward, senderID]);
                    const markResult = await markQuestsClaimed(senderID, claimAllResult.claimableIDs);

                    if (markResult.markedCount !== claimAllResult.claimableCount || markResult.totalReward !== claimAllResult.totalReward) {
                ...
                const acceptedQuests = latestData.quests.filter((quest) => quest.accepted);
                if (acceptedQuests.length === 0) {
                    await connection.rollback();
                    return api.sendMessage("📭 Bạn chưa nhận nhiệm vụ nào. Dùng quest nhan để lấy quest random.", threadID, messageID);
                }
                ...
                if (sttList.length === 0) {
                    await connection.rollback();
                    return api.sendMessage("⚠️ STT không hợp lệ. Ví dụ: quest claim 1 2 3", threadID, messageID);
                }

                const uniqueStt = [...new Set(sttList)];
                const invalidStt = uniqueStt.filter((num) => num < 1 || num > acceptedQuests.length);
                ...
                const selectedQuestIDs = uniqueStt.map((stt) => acceptedQuests[stt - 1].id);
                const preview = await previewClaims(senderID, selectedQuestIDs);

                if (preview.totalReward > 0) {
                    await connection.execute("UPDATE messenger_users SET credits = credits + ? WHERE psid = ?", [preview.totalReward, senderID]);
                    const markResult = await markQuestsClaimed(senderID, preview.claimableIDs);
                    if (markResult.markedCount !== preview.claimableCount || markResult.totalReward !== preview.totalReward) {
                        throw new Error("Quest claim mismatch after DB update");
                    }
                }
                ...
                if (preview.alreadyClaimedCount > 0) rewardMsg += `ℹ️ Đã nhận trước đó: ${preview.alreadyClaimedCount}\n`;
                if (preview.notCompleteCount > 0) rewardMsg += `⏳ Chưa hoàn thành: ${preview.notCompleteCount}\n`;
                ...
                return api.sendMessage("❌ Lỗi nhận thưởng quest.", threadID, messageID);
            } finally {
        ...
        if (subCommand === "check") {
            ...
            if (acceptedQuests.length === 0) {
                return api.sendMessage("📭 Bạn chưa nhận nhiệm vụ nào. Gõ quest nhan để lấy quest random.", threadID, messageID);
            }
        ...
        if (["nhan", "accept", "random"].includes(subCommand)) {
            ...
            if (!result.ok) {
                return api.sendMessage("❌ Không thể nhận quest lúc này, thử lại sau.", threadID, messageID);
            }
