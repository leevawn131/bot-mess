...
module.exports = {
    name: "tien",
    description: "Xem tiền & Chuyển tiền (Fix tên 100%)",
    execute: async ({ api, event, args }) => {
        ...
        } catch (err) { return api.sendMessage("❌ Lỗi config.json", threadID); }
        ...
        try {
            ...
            const getName = async (uid) => {
                ...
                try {
                    const [rows] = await connection.execute('SELECT name FROM messenger_users WHERE psid = ?', [uid]);
                    if (rows.length > 0 && rows[0].name && rows[0].name !== "Thành viên mới") return rows[0].name;
                } catch(e) {}
            ...
            const [userRows] = await connection.execute('SELECT * FROM messenger_users WHERE psid = ?', [senderID]);
            
            if (userRows.length === 0) {
                // Người mới -> Tạo
                await connection.execute('INSERT INTO messenger_users (psid, name, credits) VALUES (?, ?, ?)', [senderID, senderName, 10000]);
                api.sendMessage(`🎉 Chào mừng ${senderName}!\n🎁 +10,000 credits.`, threadID);
            } else {
                // Người cũ -> CẬP NHẬT TÊN NẾU TÊN CŨ LÀ "THÀNH VIÊN MỚI"
                // Hoặc nếu tên hiện tại khác tên trong DB
                const dbName = userRows[0].name;
                if (senderName !== "Thành viên mới" && (dbName === "Thành viên mới" || dbName !== senderName)) {
                    await connection.execute('UPDATE messenger_users SET name = ? WHERE psid = ?', [senderName, senderID]);
                } else if (senderName === "Thành viên mới" && dbName !== "Thành viên mới") {
            ...
            if (["chuyen", "pay", "give"].includes(command)) {
                ...
                if (!targetID) return api.sendMessage("⚠️ Tag/Reply/ID người nhận.", threadID, messageID);
                if (targetID === senderID) return api.sendMessage("❌ Tự chuyển cho mình à?", threadID, messageID);

                let amount = 0;
                for (let arg of args) if (!isNaN(arg) && arg !== targetID) { amount = parseInt(arg); break; }
                if (amount <= 0) return api.sendMessage("⚠️ Số tiền sai.", threadID, messageID);
                ...
                if (targetLoanRows.length > 0) {
                    ...
                    const dueDate = new Date(takenAt.getTime() + targetLoan.due_days * 24 * 60 * 60 * 1000);
                ...
                try {
                    ...
                    if (senderID !== BOSS_ID) {
                        // Check VIP status
                        const now = new Date();
                        const hasVIP = sRows[0].vip_until && new Date(sRows[0].vip_until) > now;
                        ...
                        dailyLimit = baseLimit + Math.floor(gamesPlayed / 5) * bonusPerFiveGames;

                        // Lấy hoặc tạo transfer_limits record
                        const vnTime = new Date(now.getTime() + (7 * 60 * 60 * 1000));
                        const todayStr = vnTime.toISOString().split('T')[0];
                        
                        const [limitRows] = await connection.execute('SELECT * FROM transfer_limits WHERE psid = ?', [senderID]);
                        
                        if (limitRows.length > 0) {
                            const lastReset = new Date(limitRows[0].last_reset);
                            const lastResetVN = new Date(lastReset.getTime() + (7 * 60 * 60 * 1000));
                            ...
                            if (lastResetStr === todayStr) {
                                ...
                                transferredToday = 0;
                                await connection.execute('UPDATE transfer_limits SET transferred_today = 0, last_reset = ? WHERE psid = ?', [now, senderID]);
                            }
                        } else {
                            // Tạo record mới
                            await connection.execute('INSERT INTO transfer_limits (psid, transferred_today, last_reset) VALUES (?, 0, ?)', [senderID, now]);
                        }
                    ...
                    const [tRows] = await connection.execute('SELECT * FROM messenger_users WHERE psid = ?', [targetID]);
                    if (tRows.length === 0) {
                        await connection.rollback();
                    ...
                    if (senderID !== BOSS_ID) {
                        await connection.execute('UPDATE transfer_limits SET transferred_today = transferred_today + ? WHERE psid = ?', [amount, senderID]);
                    }
            ...
            // KIỂM TRA HẠN MỨC
                ...
                const [userRows] = await connection.execute('SELECT games_played, vip_until FROM messenger_users WHERE psid = ?', [senderID]);
                if (userRows.length === 0) return api.sendMessage("❌ Bạn chưa có tài khoản.", threadID, messageID);

                const now = new Date();
                const hasVIP = userRows[0].vip_until && new Date(userRows[0].vip_until) > now;
                const gamesPlayed = parseInt(userRows[0].games_played || 0);

                // Tính hạn mức
                const baseLimit = hasVIP ? 100000 : 50000;
                const bonusPerFiveGames = hasVIP ? 75000 : 50000;
                const dailyLimit = baseLimit + Math.floor(gamesPlayed / 5) * bonusPerFiveGames;
            ...
            // XEM SỐ DƯ
                ...
                const [rows] = await connection.execute('SELECT credits, name FROM messenger_users WHERE psid = ?', [viewID]);
                if (rows.length === 0) return api.sendMessage(`❌ ${viewName} chưa có tài khoản.`, threadID, messageID);

                // Ưu tiên hiển thị tên vừa lấy được (viewName) cho chuẩn xác nhất
                const finalName = (viewName !== "Thành viên mới") ? viewName : rows[0].name;

                return api.sendMessage(`💰 TÀI KHOẢN: ${finalName}\n💳 Số dư: ${parseInt(rows[0].credits).toLocaleString()} credits`, threadID, messageID);
            }
