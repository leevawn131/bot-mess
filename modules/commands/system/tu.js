...
module.exports = {
    ...
    execute: async ({ api, event, args }) => {
        ...
        // Cooldown 5s
        const cooldown = checkCooldown({ command: "tu", key: senderID, durationMs: 5000 });
        if (!cooldown.allowed) {
        ...
        try {
            ...
            if (command === "check" && args[1] === "all") {
                const [rows] = await connection.execute(
                    'SELECT uj.psid, uj.jail_until, uj.reason, mu.name FROM user_jail uj LEFT JOIN messenger_users mu ON uj.psid = mu.psid WHERE uj.jail_until > NOW()'
                );
            ...
            if (command === "cuu") {
                ...
                if (credits < fee) {
                    return api.sendMessage(
                        `💸 KHÔNG ĐỦ TIỀN CỨU TÙ!\n━━━━━━━━━━━━━━━━━━\n💰 Phí: ${fee.toLocaleString()}\n💵 Bạn có: ${credits.toLocaleString()}\n💔 Thiếu: ${(fee - credits).toLocaleString()}`,
                        threadID,
                        messageID
                    );
                }
                ...
                try {
                    // Trừ tiền user
                    await connection.execute('UPDATE messenger_users SET credits = credits - ? WHERE psid = ?', [fee, senderID]);

                    // Thả tù cho người được cứu
                    await connection.execute('DELETE FROM user_jail WHERE psid = ?', [targetID]);
