...
module.exports = {
    ...
    execute: async ({ api, event, args, config }) => {
        ...
        // Cooldown 10s
        const cooldown = checkCooldown({ command: "baucua", key: threadID, durationMs: 10000 });
        if (!cooldown.allowed) {
        ...
        let dbConfig = null;
        if (dbInfo) {
             dbConfig = { host: dbInfo.host, port: dbInfo.port, user: dbInfo.user, password: dbInfo.password, database: dbInfo.name };
        } else {
        ...
        if (command === "xoc" || command === "so") {
            if (!global.baucuaSessions[threadID]) return api.sendMessage("❌ Chưa mở phiên nào.", threadID);
            
            const session = global.baucuaSessions[threadID];
            const players = Object.values(session.players);

            if (players.length === 0) {
                delete global.baucuaSessions[threadID];
                return api.sendMessage("⚠️ Phiên hủy do vắng khách.", threadID);
            }
            ...
            let msg = `🎰 KẾT QUẢ (Phiên #${session.sessionID}): ${resultIcons}\n━━━━━━━━━━━━━━━━━━\n`;
        ...
        // --- LỆNH: !baucua (MỞ PHIÊN) ---
            ...
            if (global.baucuaSessions[threadID]) return api.sendMessage("⚠️ Đang có phiên rồi! Gõ !baucua xoc để chốt.", threadID);
    ...
    handleReply: async ({ api, event }) => {
        ...
        try {
            ...
            const [rows] = await connection.execute('SELECT credits, name FROM messenger_users WHERE psid = ?', [senderID]);
            if (rows.length === 0) return api.sendMessage("❌ Bạn chưa có tài khoản.", threadID, messageID);
            ...
            if (isNaN(betAmount) || betAmount <= 0) return api.sendMessage("⚠️ Tiền cược không hợp lệ.", threadID, messageID);
