...
module.exports = {
    ...
    usage: "!lode [số 00-99] [tiền cược]",
    
    execute: async ({ api, event, args, config }) => {
        const { threadID, senderID, messageID } = event;

        // Cooldown 10s
        const cooldown = checkCooldown({ command: "lode", key: threadID, durationMs: 10000 });
        if (!cooldown.allowed) {
            return api.sendMessage(`⏳ Vui lòng chờ ${cooldown.timeLeft}s trước khi chơi tiếp!`, threadID, messageID);
        }
        ...
        if (!pick || isNaN(pick) || pick < 0 || pick > 99 || pick.length !== 2) {
            return api.sendMessage("⚠️ Vui lòng nhập số từ 00-99.\nVí dụ: !lode 88 10000", threadID, messageID);
        }
        ...
        try {
            ...
            if (rows.length === 0) return api.sendMessage("❌ Bạn chưa có tài khoản (!diemdanh để nhận tiền).", threadID, messageID);
            ...
            if (isNaN(amount) || amount <= 0) return api.sendMessage("⚠️ Tiền cược không hợp lệ.", threadID, messageID);
            if (balance < amount) return api.sendMessage(`💸 Không đủ tiền! Còn: ${balance.toLocaleString()}`, threadID, messageID);
            ...
            // --- QUAY SỐ ---
            api.sendMessage(`🎰 Đang quay số... Chúc bạn may mắn với con số **${pick}**...`, threadID);
            ...
            const result = isWin ? pick : Math.floor(Math.random() * 100).toString().padStart(2, '0');
            ...
            if (result === pick) {
                ...
                await connection.execute('UPDATE messenger_users SET credits = credits + ? WHERE psid = ?', [winMoney, senderID]);
                
                // Trừ tiền của Boss (Vì Boss phải trả thưởng)
                const bossLoss = winMoney - amount; 
                await connection.execute('UPDATE messenger_users SET credits = credits - ? WHERE psid = ?', [bossLoss, BOSS_ID]);
                ...
                if (hasVIP) {
                    ...
                    await connection.execute('UPDATE messenger_users SET credits = credits + ? WHERE psid = ?', [refund, senderID]);
                ...
                await connection.execute('UPDATE messenger_users SET credits = credits + ? WHERE psid = ?', [amount - refund, BOSS_ID]);
