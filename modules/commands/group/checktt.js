...
async function sendMessageSafe(api, body, threadID, replyToMessageID) {
    try {
        ...
        try {
            ...
            const resultRetry = await api.sendMessage(body, threadID, replyToMessageID);
            if (resultRetry && typeof resultRetry === "object") return { ok: true, info: resultRetry };
            return { ok: true, info: null };
        } catch (e2) {
...
function buildRankList({ threadInfo, threadStats, threshold }) {
    ...
    const allIDs = new Set([...Object.keys(threadStats || {}), ...participantSet]);
...
module.exports = {
    ...
    usage: "[all | locmem <số_tin_tổng> | clear | reset | kickdead]",

    execute: async ({ api, event, args }) => {
        const { threadID, messageID, senderID } = event;

        const cooldown = checkCooldown({ command: "checktt", key: senderID, durationMs: 10000 });
        if (!cooldown.allowed) {
        ...
        try {
            ...
            if (!canUseChecktt) {
                return api.sendMessage("⚠️ Lệnh checktt chỉ dành cho QTV nhóm hoặc chủ bot.", threadID, messageID);
            }
            ...
            if (sub === "reset") {
                if (!canUseChecktt) {
                    return api.sendMessage("⚠️ Chỉ QTV nhóm hoặc chủ bot mới được reset dữ liệu tương tác.", threadID, messageID);
                }

                stats[threadID] = {};
                writeStats(stats);
                return api.sendMessage("✅ Đã reset dữ liệu tin nhắn của nhóm này.", threadID, messageID);
            }

            if (sub === "clear") {
                if (!canUseChecktt) {
                    return api.sendMessage("⚠️ Chỉ QTV nhóm hoặc chủ bot mới được clear dữ liệu tương tác.", threadID, messageID);
                }

                const participantSet = new Set((threadInfo.participantIDs || []).map(id => String(id)));
                const allStatIDs = Object.keys(threadStats || {});
                const removedIDs = allStatIDs.filter(uid => !participantSet.has(String(uid)));

                if (removedIDs.length === 0) {
                    return api.sendMessage("📭 Không có thành viên đã rời nhóm trong danh sách để xóa.", threadID, messageID);
                }
                ...
                writeStats(stats);
                return api.sendMessage(`🧹 Đã clear ${removedIDs.length} thành viên đã rời nhóm khỏi dữ liệu tương tác.`, threadID, messageID);
            }

            if (sub === "kickdead") {
                if (!canUseChecktt) {
                    return api.sendMessage("⚠️ Chỉ QTV nhóm hoặc chủ bot mới được kickdead.", threadID, messageID);
                }
                if (!isBotAdmin) {
                    return api.sendMessage("❌ Bot cần quyền QTV để kick thành viên.", threadID, messageID);
                }
                ...
                if (candidates.length === 0) {
                    return api.sendMessage("📭 Không có tài khoản 'Người dùng Facebook' nào trong danh sách tương tác.", threadID, messageID);
                }
                ...
                for (const user of candidates) {
                    ...
                    try {
                        ...
                        failed++;
                        failedDetails.push(`- ${user.name || uid} (${uid}): ${e?.message || "kick lỗi"}`);
                    }
                }

                let msg = `✅ kickdead xong.\n• Tìm thấy: ${candidates.length}\n• Thành công: ${success}\n• Thất bại: ${failed}`;
            ...
            if (sub === "locmem") {
                ...
                if (isNaN(threshold) || threshold < 0) {
                    return api.sendMessage("⚠️ Dùng đúng cú pháp: checktt locmem <số_tin_tổng>", threadID, messageID);
                }

                if (!isBotAdmin) {
                    return api.sendMessage("❌ Bot cần quyền QTV để xóa thành viên bằng locmem.", threadID, messageID);
                }
                ...
                if (candidates.length === 0) {
                    return api.sendMessage(`📭 Không có thành viên nào có tổng tin nhắn <= ${threshold}.`, threadID, messageID);
                }

                suppressLeaveEvents(threadID, Math.max(15000, candidates.length * 1200));
                ...
                for (const user of candidates) {
                    ...
                    try {
                        ...
                        failed++;
                        failedDetails.push(`- ${user.name || uid} (${uid}): ${e?.message || "xóa lỗi"}`);
                    }
                }

                writeStats(stats);

                let msg = `✅ locmem xong (tổng tin <= ${threshold}).\n• Tìm thấy: ${candidates.length}\n• Thành công: ${success}\n• Thất bại: ${failed}`;
                if (failedDetails.length > 0) {
                    msg += `\n\n📌 Chi tiết lỗi:\n${failedDetails.slice(0, 10).join("\n")}`;
                }
            ...
            if (!sub) {
                ...
                const getRank = (sortedList, uid) => {
                    const idx = sortedList.findIndex(item => String(item.uid) === String(uid));
                ...
                const scopeText = String(event.messageReply?.senderID) === targetID ? "(người được reply)" : "(bạn)";

                let msg = `📊 CHECKTT CÁ NHÂN ${scopeText}`;
                msg += `\n👤 ${safeTarget.name}`;
                msg += `\n━━━━━━━━━━━━━━━━━━`;
                msg += `\n• Ngày: hạng ${dayRank}/${rankedBase.length} - ${safeTarget.dayCount} tin`;
                msg += `\n• Tuần: hạng ${weekRank}/${rankedBase.length} - ${safeTarget.weekCount} tin`;
                msg += `\n• Tháng: hạng ${monthRank}/${rankedBase.length} - ${safeTarget.monthCount} tin`;
                msg += `\n• Tổng: hạng ${totalRank}/${rankedBase.length} - ${safeTarget.count} tin`;
                msg += `\n\n📌 Gợi ý: checktt all để xem danh sách tổng như cũ.`;

                return api.sendMessage(msg, threadID, messageID);
            }

            if (sub !== "all") {
                return api.sendMessage("⚠️ Cú pháp không hợp lệ. Dùng: checktt hoặc checktt all", threadID, messageID);
            }
            ...
            const title = "📊 CHECKTT ALL - TOP TỔNG TIN NHẮN";

            const lines = [
                `${title}`,
                "━━━━━━━━━━━━━━━━━━",
                ...ranked.map((item, idx) => {
                    const status = item.inGroup ? "" : " (đã rời nhóm)";
                    return `${idx + 1}. ${item.name}${status} — ${item.count} tin`;
                }),
                "",
                "↩️ Reply nhiều STT (vd: 1 2 3 hoặc 1,2,3; tối đa 5) để xử lý (kick hoặc xóa data nếu đã rời nhóm)",
                "",
                "📌 Lệnh nhanh:",
                "• checktt → xem thứ hạng cá nhân theo ngày/tuần/tháng",
                "• checktt locmem <số> → xóa mem có tổng tin nhắn <= số đó",
                "• checktt clear → xóa dữ liệu thành viên đã rời nhóm",
                "• checktt reset → reset dữ liệu tương tác của nhóm này",
                "• checktt kickdead → kick tài khoản 'Người dùng Facebook' có trong dữ liệu"
            ];
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                if (!sent.ok) {
                    return api.sendMessage("❌ Facebook đang lỗi tạm thời, thử lại sau vài giây.", threadID, messageID);
                }
    ...
    handleReply: async ({ api, event }) => {
        try {
            ...
            if (String(context.author) !== String(senderID)) {
                return api.sendMessage("⚠️ Chỉ người gọi checktt mới được reply STT.", threadID, messageID);
            }
            ...
            if (sttList.length === 0) {
                return api.sendMessage("⚠️ STT không hợp lệ. Ví dụ: 1 2 3 hoặc 1,2,3", threadID, messageID);
            }

            if (sttList.length > 5) {
                return api.sendMessage("⚠️ Mỗi lần chỉ xử lý tối đa 5 STT.", threadID, messageID);
            }

            if (sttList.some(stt => stt < 1 || stt > context.ranked.length)) {
                return api.sendMessage("⚠️ Có STT vượt ngoài danh sách.", threadID, messageID);
            }
            ...
            if (!isSenderAdmin && !isSenderBotAdmin) {
                return api.sendMessage("⚠️ Chỉ QTV nhóm hoặc chủ bot mới được xử lý kick/xóa bằng STT.", threadID, messageID);
            }
            ...
            const hasKickCandidate = selectedTargets.some((target) => {
                ...
                return participantSet.has(uid) && uid !== botID && !adminIDs.includes(uid);
            });
            ...
            for (const target of selectedTargets) {
                ...
                if (participantSet.has(uid)) {
                    ...
                    try {
                        ...
                        failed++;
                        failedDetails.push(`- ${target.name || uid}: ${e?.message || "kick lỗi"}`);
                    }
                    continue;
                }
