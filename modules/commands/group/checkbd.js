    ...
    try {
      ...
      if (sub === "canhbao") {
        ...
        const warningLines = [
          ...
          ...nonAdminMissingList.map((item, idx) => {
              ...
              mentions: [
                {
                  id: String(item.uid),
                  tag: tagText,
                  fromIndex: `${idx + 1}. `.length,
                },
              ],
            };
          }),
          { text: "" },
          { text: "📌 Vui lòng setbd sớm để dễ gọi tên và tránh bị nhắc lại." },
        ];

        const warningChunks = chunkRichLines(warningLines);
        for (const chunk of warningChunks) {
          await sendMessageSafe(
            api,
            { body: chunk.body, mentions: chunk.mentions },
            threadID,
            messageID,
          );
        }
