...
module.exports = {
    ...
    usage: "[kickall] (hoặc reply STT để kick)",

    execute: async ({ api, event, args }) => {
        const { threadID, messageID, senderID } = event;

        const cooldown = checkCooldown({ command: "checkbd", key: senderID, durationMs: 10000 });
        if (!cooldown.allowed) {
        ...
        try {
            ...
            if (!isSenderAdmin && !isSenderBotAdmin) {
                return api.sendMessage("⚠️ Lệnh checkbd chỉ dành cho QTV nhóm hoặc chủ bot.", threadID, messageID);
            }

            const missingList = buildMissingNicknameList(threadInfo, botID);

            if (missingList.length === 0) {
                return api.sendMessage("✅ Tất cả thành viên hiện tại đã set biệt danh.", threadID, messageID);
            }
            ...
            if (sub === "kickall") {
                ...
                const candidates = missingList.filter((item) => !adminIDs.includes(String(item.uid)));

                if (candidates.length === 0) {
                    return api.sendMessage("🛡️ Tất cả người chưa set biệt danh đều là QTV, không thể kick.", threadID, messageID);
                }
                ...
                for (const user of candidates) {
                    ...
                    try {
                        ...
                        failed++;
                        failedDetails.push(`- ${user.name || uid}: ${e?.message || "kick lỗi"}`);
                    }
                }
            ...
            const lines = [
                ...
                "• Bot sẽ tự bỏ qua QTV và chính bot",
                "",
                "📌 Lệnh nhanh:",
    ...
    handleReply: async ({ api, event }) => {
        try {
            ...
            if (String(context.author) !== String(senderID)) {
                return api.sendMessage("⚠️ Chỉ người gọi checkbd mới được reply STT.", threadID, messageID);
            }
            ...
            if (!isSenderAdmin && !isSenderBotAdmin) {
                return api.sendMessage("⚠️ Chỉ QTV nhóm hoặc chủ bot mới được xử lý kick bằng STT.", threadID, messageID);
            }
